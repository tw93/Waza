...
when_to_use: "设计, 做页面, 做组件, 不好看, 不和谐, 不清晰, 很丑, 很怪, 很傻, 突兀, 不协调, 字体, 字形, 排印, 排版, 样式, 前端, UI, 截图, 截图还原, 视觉, 还原度, visual, design, layout, styling, frontend, component, typography, responsive, css, tailwind, build page, create component, make it look good, style, design, screenshot with visual complaint, typography, font looks wrong, 못생겼다, 이상하다, 어색하다, 안 맞다, 조화롭지 않다"
...
**Chinese/Korean gut-feel complaints**: when the user says "很傻", "很怪", "突兀", "不协调", "不和谐" (Chinese), or "못생겼다", "이상하다", "어색하다", "안 맞다", "조화롭지 않다" (Korean) about a visual, treat it as an aesthetic rejection, not a debugging symptom. Route to Screenshot Iteration Mode, not to `/hunt`.
